...
command=php -d memory_limit=-1 artisan queue:listen
...
EOB
  ...
  sudo echo -e '#!/usr/bin/env bash\nservice supervisord stop' > /opt/elasticbeanstalk/hooks/appdeploy/post/00_stop_supervisord.sh
  sudo chmod +x /opt/elasticbeanstalk/hooks/appdeploy/post/00_stop_supervisord.sh
  # then right after the webserver is reloaded, we can start supervisord again
  sudo echo -e '#!/usr/bin/env bash\nservice supervisord start' > /opt/elasticbeanstalk/hooks/appdeploy/post/99_z_start_supervisord.sh
  sudo chmod +x /opt/elasticbeanstalk/hooks/appdeploy/post/99_z_start_supervisord.sh

  sudo sh /opt/elasticbeanstalk/hooks/appdeploy/post/99_z_start_supervisord.sh
fi
